...
public class PathMaker {
	...
	// Digged Rect
	//==========================================================================================
	private static const DIGGED_RECT_COMMAND:Vector.<int> = new <int>[1, 2, 2, 2, 2, 2, 2];

	public static function diggedRect(x:Number, y:Number, width:Number, height:Number, digX:Number, digY:Number, isUpperDig:Boolean, isLeftDig:Boolean):GraphicsPath {
		var path:GraphicsPath = new GraphicsPath;
		path.commands = DIGGED_RECT_COMMAND;

		var rectData:Vector.<Number> = new Vector.<Number>(10, true);
		rectData[0] = x;
		rectData[1] = y;
		rectData[2] = x + width;
		rectData[3] = y;
		...
		var digData:Vector.<Number>;
		// TL
		if (isUpperDig && isLeftDig) {
			digData = new Vector.<Number>().concat(new <Number>[x, digY, digX, digY, digX, y], rectData.slice(2, 8), new <Number>[x, digY]);
		}
		// TR
		else if (isUpperDig && !isLeftDig) {
			digData = new Vector.<Number>().concat(rectData.slice(0, 2), new <Number>[digX, y, digX, digY, x + width, digY], rectData.slice(4));
		}
		// BR
		else if (!isUpperDig && !isLeftDig) {
			digData = new Vector.<Number>().concat(rectData.slice(0, 4), new <Number>[x + width, digY, digX, digY, digX, y + height], rectData.slice(6));
		}
		// BL
		else if (!isUpperDig && isLeftDig) {
			digData = new Vector.<Number>().concat(rectData.slice(0, 6), new <Number>[digX, y + height, digX, digY, x, digY, x, y]);
		}

		if (!path.data) path.data = digData;
